...
export async function getInvoices(profileId) {
  const r = await fetch(`/api/profiles/${profileId}/invoices`);
  return r.json();
}

export async function extractInvoice(profileId, file) {
  const fd = new FormData();
  fd.append("file", file);
  fd.append("profileId", profileId);
...
export async function saveInvoice(profileId, payload) {
  const r = await fetch(`/api/profiles/${profileId}/invoices`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
...
export async function deleteInvoice(profileId, invoiceId) {
  await fetch(`/api/profiles/${profileId}/invoices/${invoiceId}`, { method: "DELETE" });
}
