    ...
    <div>
      ...
      {confirmDelete && (
        <ConfirmModal
          ...
          message="Isso apaga o perfil e todo o histórico de exames, notas fiscais e PDFs guardados dessa pessoa. Essa ação não pode ser desfeita."
      ...
      {!result ? (
        <>
          <p className="text-xs text-slate-500 mb-4">
            Selecione o arquivo <code>backup-exames-....json</code> exportado do artefato do Claude. Os perfis, exames e notas fiscais dele serão adicionados aos que já existem aqui.
          </p>
    ...
    <div>
      ...
      <div className="flex items-center gap-1 border-b border-slate-200 mb-5">
        <button
          ...
          className={`text-sm font-medium px-3 py-2 border-b-2 -mb-px transition ${tab === "exames" ? "border-slate-900 text-slate-900" : "border-transparent text-slate-400 hover:text-slate-600"}`}
        >
          Exames
        </button>
        <button
          onClick={() => setTab("notas")}
          className={`flex items-center gap-1.5 text-sm font-medium px-3 py-2 border-b-2 -mb-px transition ${tab === "notas" ? "border-slate-900 text-slate-900" : "border-transparent text-slate-400 hover:text-slate-600"}`}
        >
...
function InvoicesPanel({ profileId }) {
  ...
  const saveInvoiceHandler = async (data) => {
    const { invoiceId } = await api.saveInvoice(profileId, data);
    setInvoices((prev) => [{ id: invoiceId, hasPdf: !!data.base64, ...data }, ...(prev || [])].sort((a, b) => (b.date || "").localeCompare(a.date || "")));
    setReviewData(null);
  };
