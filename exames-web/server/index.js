...
import db, { pdfDir, invoiceDir } from "./db.js";
import { callClaude, parseExamJson, extractJsonBlock, EXTRACTION_PROMPT, INVOICE_EXTRACTION_PROMPT } from "./anthropic.js";
...
app.delete("/api/profiles/:id", (req, res) => {
  ...
  const invoices = db.prepare("SELECT id FROM invoices WHERE profile_id = ?").all(id);
  for (const inv of invoices) {
    const invPath = path.join(invoiceDir, `${inv.id}.pdf`);
...
app.get("/api/profiles/:profileId/invoices", (req, res) => {
  ...
  const rows = db
    .prepare(
      `SELECT id, date, provider, doc, value, category, description, deduct,
              file_hash as hash, has_pdf as hasPdf, pdf_filename as fileName, saved_at as savedAt
       FROM invoices WHERE profile_id = ? ORDER BY date DESC`
    )
    .all(profileId)
    .map((r) => ({ ...r, deduct: !!r.deduct, hasPdf: !!r.hasPdf }));
  res.json(rows);
});

app.get("/api/profiles/:profileId/invoices/summary", (req, res) => {
  const { profileId } = req.params;
  const { year } = req.query;
  let rows = db.prepare("SELECT date, value, category, deduct FROM invoices WHERE profile_id = ?").all(profileId);
  ...
  const deductible = rows.filter((r) => r.deduct);
  const total = deductible.reduce((s, r) => s + (r.value || 0), 0);
  const byCategory = {};
  for (const r of deductible) {
    byCategory[r.category || "Outro"] = (byCategory[r.category || "Outro"] || 0) + (r.value || 0);
  }
  res.json({ total, count: deductible.length, byCategory, years: allYears });
});
...
app.get("/api/export", (req, res) => {
  try {
    ...
    const backup = { version: 2, exportedAt: new Date().toISOString(), profiles, batches: {}, invoices: {} };
    for (const p of profiles) {
      ...
      const invoiceRows = db
        .prepare(
          `SELECT id as invoiceId, date, provider, doc, value, category, description, deduct, file_hash as hash, pdf_filename as fileName
           FROM invoices WHERE profile_id = ?`
        )
        .all(p.id);
      const invoiceList = [];
      for (const inv of invoiceRows) {
        const pdfPath = path.join(invoiceDir, `${inv.invoiceId}.pdf`);
        let pdfBase64 = null;
        if (fs.existsSync(pdfPath)) {
          pdfBase64 = fs.readFileSync(pdfPath).toString("base64");
        }
        invoiceList.push({ ...inv, deduct: !!inv.deduct, pdfBase64 });
      }
      backup.invoices[p.id] = invoiceList;
    }
    res.json(backup);
  } catch (e) {
...
app.post("/api/import", (req, res) => {
  try {
    const { profiles, batches, invoices } = req.body || {};
    ...
    res.json({ ok: true, importedProfiles, importedBatches, importedResults, importedInvoices });
  } catch (e) {
